+function ( $, window) {
  ...
  function Flumns(element, options) {
    ...
    function layout() {
      ...
      // var h = getClientRect(element).height;
      // $element.css('height', h + "px");
      // console.log("height: ", h, $element.height(), element);
    ...
    function resizeHandler(event) {
      focusedElement = $element.find(':focus')
      if (focusedElement.length === 0) {
        layout.call(instance);
      } else {
        focusedElement.one('blur', resizeHandler)
      }
    }
